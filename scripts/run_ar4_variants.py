#!/usr/bin/env python
"""Execute all AR-4 configuration variants sequentially."""
...
AR4_CONFIG_DIR = ANALYSIS_CONFIG_DIR / "AR4_dwell_times"


def _discover_variant_configs() -> Iterable[str]:
    if not AR4_CONFIG_DIR.exists():
        return []

    for path in sorted(AR4_CONFIG_DIR.glob("*.yaml")):
...
def main() -> int:
    ...
    if not variants:
        print("No AR-4 variant configuration files were found.", file=sys.stderr)
        return 1

    print(f"Discovered {len(variants)} AR-4 variants.")
    exit_code = 0

    for config_name in variants:
        print(f"\n=== Running AR-4 variant: {config_name} ===")
        try:
            cfg = load_config(overrides=[f"analysis_specific.ar4_dwell_times.config_name={config_name}"])
            metadata = ar4.run(config=cfg)
