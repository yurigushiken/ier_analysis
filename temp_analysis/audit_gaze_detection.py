"""Audit gaze fixation detection logic."""
...
# Manually detect gaze fixations using same logic as gaze_detector.py
print(f"\n2. Manually detecting gaze fixations (3+ consecutive frames on same AOI)")
...
df_gaze = pd.DataFrame(gaze_fixations)
print(f"   Detected gaze fixations: {len(df_gaze)}")
print(f"   Average frames per gaze fixation: {df_gaze['frames'].mean():.1f}")
print(f"   Median frames per gaze fixation: {df_gaze['frames'].median():.1f}")
print(f"   Range: {df_gaze['frames'].min()} - {df_gaze['frames'].max()} frames")

print(f"\n3. Gaze fixations by AOI category:")
print(df_gaze.groupby('aoi')['frames'].agg(['count', 'mean', 'sum']))

print(f"\n4. Gaze fixations by trial:")
trial_summary = df_gaze.groupby('trial').agg({'aoi': 'count', 'frames': 'sum'})
trial_summary.columns = ['n_gaze_fixations', 'total_frames_in_gazes']
print(trial_summary)

# Compare with processed file
print(f"\n5. Comparing with processed gaze_fixations_child.csv")
df_processed = pd.read_csv('data/processed/gaze_fixations_child.csv')
participant_id = df_raw['Participant'].iloc[0]
df_processed_participant = df_processed[df_processed['participant_id'] == participant_id]

print(f"   Processed file - this participant: {len(df_processed_participant)} gaze fixations")
print(f"   Manual detection - this participant: {len(df_gaze)} gaze fixations")
